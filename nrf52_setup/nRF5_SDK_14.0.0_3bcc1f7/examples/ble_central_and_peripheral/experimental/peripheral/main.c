		...
		if (c == 'p' || c == 's') {
			...
			for (uint32_t i = 0; i < TOF_SAMPLES_TO_ACQUIRE; i++) {
				if (c == 'p') {
					...
					NRF_LOG_INFO("i = %u ifs_duration_ticks = %u ticks rssi = %d dBm radio_freq = 0x%08x\n\r", i,
							p_ctx->buffer[i].ifs_duration_ticks, p_ctx->buffer[i].rssi,
							p_ctx->buffer[i].frequency_reg);
#else
					NRF_LOG_INFO("i = %u ifs_duration_ticks = %u ns rssi = %d dBm radio_freq = 0x%08x\n\r", i,
							p_ctx->buffer[i].ifs_duration_ticks, p_ctx->buffer[i].rssi,
							p_ctx->buffer[i].frequency_reg);
#endif
				}
			...
			//TODO: it should be better to use NRF_LOG_DEFERRED, but when in use the mcu locks when nobody reads from uart
			//TODO: I changed the source code of nrf_log_frontend.c to fix the problem of NRF_LOG_DEFERRED. The code has been written by a member of the forum, not by nordic
			char line[300];
#ifdef USE_NANOSECOND
			float ifs_duration_ns_comp_avg = (ifs_duration_ticks_avg - ifs_offset_ticks)/(((float)IFS_TIMER_TICK_FREQUENCY_MHZ) / 1000);
			if(TOF_SAMPLES_TO_ACQUIRE == 1){
				sprintf(line, "bd_addr = %s ifs_duration_ns = "NRF_LOG_FLOAT_MARKER" ns rssi = %d dBm radio_freq = 0x%08x c = %d\n\r",addr_str,NRF_LOG_FLOAT(ifs_duration_ns_comp_avg), p_ctx->buffer[0].rssi,(unsigned int)p_ctx->buffer[0].frequency_reg,count++);
				NRF_LOG_INFO("%s",nrf_log_push(line));
				//NRF_LOG_RAW_INFO("ifs_duration_ns = "NRF_LOG_FLOAT_MARKER" ns rssi = "NRF_LOG_FLOAT_MARKER" dBm",NRF_LOG_FLOAT(ifs_duration_ns_comp_avg), NRF_LOG_FLOAT(rssi_avg));
				//NRF_LOG_RAW_INFO(" radio_freq = 0x%08x c = %d\n\r",p_ctx->buffer[0].frequency_reg,count++);
			}else{
				sprintf(line, "bd_addr = %s ifs_duration_ns = "NRF_LOG_FLOAT_MARKER" ns rssi = "NRF_LOG_FLOAT_MARKER" dBm c = %d\n\r",addr_str,NRF_LOG_FLOAT(ifs_duration_ns_comp_avg), NRF_LOG_FLOAT(rssi_avg),count++);
				NRF_LOG_INFO("%s",nrf_log_push(line));
...
#else
			if(TOF_SAMPLES_TO_ACQUIRE == 1){
				//NRF_LOG_RAW_INFO("ifs_duration_ticks = %d rssi = %d dBm", ((int32_t)p_ctx->buffer[0].ifs_duration_ticks)-ifs_offset_ticks, p_ctx->buffer[0].rssi);
				//NRF_LOG_RAW_INFO(" radio_freq = 0x%08x\n\r",p_ctx->buffer[0].frequency_reg);
				sprintf(line, "bd_addr = %s ifs_duration_ticks = %d ticks rssi = %d dBm radio_freq = 0x%08x c = %d\n\r",addr_str,((int32_t)p_ctx->buffer[0].ifs_duration_ticks)-ifs_offset_ticks, p_ctx->buffer[0].rssi,(unsigned int)p_ctx->buffer[0].frequency_reg,count++);
				NRF_LOG_INFO("%s",nrf_log_push(line));
			}else{
				...
				sprintf(line, "bd_addr = %s ifs_duration_ticks = "NRF_LOG_FLOAT_MARKER" ticks rssi = "NRF_LOG_FLOAT_MARKER" dBm\n\r",addr_str,NRF_LOG_FLOAT(ifs_duration_ticks_avg), NRF_LOG_FLOAT(rssi_avg));
				NRF_LOG_INFO("%s",nrf_log_push(line));
			}
#endif
			...
			if (!m_test_params.continuous_test) {
				...
				for(uint8_t idx = 0; idx < TOF_SAMPLES_TO_ACQUIRE; idx++){
					NRF_LOG_INFO("idx = %u", p_ctx->buffer[idx].connection_events_counter);
				}
			}
		}
	}
...
//	tof_samples = TOF_SAMPLES_TO_ACQUIRE;
	if (!m_test_params.continuous_test) {
		test_terminate();
	}
...
static void test_run(void)
{
	//memset(m_ifs_tof.buffer, 0x00, TOF_SAMPLES_TO_ACQUIRE); //reset the buffer, create a function in ifs_tof module
...
//#endif
    ...
    NRF_LOG_INFO("Press button 3 for a continuous test.\n\r");
    NRF_LOG_INFO("Press button 4 for a single test.\n\r");

    NRF_LOG_INFO("Remember: every sample plotted in the terminal corresponds to %u samples averaged.\n\r",TOF_SAMPLES_TO_ACQUIRE);
    NRF_LOG_INFO("The average size is set by TOF_SAMPLES_TO_ACQUIRE in ifs_tof.h\n\r");
