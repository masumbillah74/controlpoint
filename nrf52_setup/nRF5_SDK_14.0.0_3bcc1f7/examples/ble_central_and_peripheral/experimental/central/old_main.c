...
uint32_t totalDevices = 6;
uint32_t previous_time[6] ={1,1,1,1,1,1};
uint32_t current_time[6] ={1,1,1,1,1,1};
uint32_t duration_time[6] ={0,0,0,0,0,0};
char *devices[6] = {"DC:E0:4E:33:35:C1", "F1:5A:2F:29:47:1A", "CC:9C:FA:C5:56:95", "C8:45:CB:50:20:25", "C0:21:D3:D6:6E:4D", "E8:F8:7B:F4:73:11"};
...
//	if (m_board_role == BOARD_TESTER)
	{
		...
		if (c == 'p' || c == 's') 
		{
			...
			//if(strcmp(addr_str, devices[currentDevice]) != 0) return;
			//currentDevice = currentDevice + 1;
			//if(currentDevice==totalDevices) currentDevice = 0;
			...
			if(TOF_SAMPLES_TO_ACQUIRE == 1)
			{
				...
				//int i = currentDevice-1;

				int i;
				for(i = 0; i<totalDevices; i++)
				{

					if(strcmp(addr_str, devices[i]) == 0) break;

				}

				current_time[i] = app_timer_cnt_get();
				duration_time[i] = app_timer_cnt_diff_compute(current_time[i], previous_time[i]);	

				//sprintf(line, ", %s, "NRF_LOG_FLOAT_MARKER", %d, %u\n\r",addr_str,NRF_LOG_FLOAT(ifs_duration_ns_comp_avg), p_ctx->buffer[0].rssi, duration_time[i]);
				sprintf(line, ": %d, "NRF_LOG_FLOAT_MARKER", %d, %u:", i, NRF_LOG_FLOAT(ifs_duration_ns_comp_avg), p_ctx->buffer[0].rssi, duration_time[i]);

				
				NRF_LOG_INFO("%s",nrf_log_push(line));

				previous_time[i] = app_timer_cnt_get();

				uint8_t adv_data[] = "aaaaaaaaaaaaaaaaaaaaaaa";	

				for(int i=0;i<sizeof(line);i++)
				{
					adv_data[i] = (uint8_t)(line[i]);
				}				

				manuf_specific_data.data.p_data = adv_data;
		  	manuf_specific_data.data.size   = sizeof(adv_data);
...
static void advertising_init(void)
{
    ...
    m_adv_params.timeout     = 0;       // 0 for Never time out.
}
